...
back into a validated config, and how it drives the base fallback, a
bridge that submits a whole form and the console bridge for that one
form. How a bridge shows a form is tested in the wizard_ui_bridge
package.
"""
...
class _FormCall(NamedTuple):
    """What the wizard passed to the latest ask_form call."""
...
class _FormReturnBridge(WizardUiBridge):
    ...
    def ask_form(self, long_question: str, ask_fields: AskFields, *,
        ...
        self.last = _FormCall(long_question, tuple(ask_fields),
                              partial_validator)
...
def test_form_names_format() -> None:
    ...
    assert 'CSV' in bridge.last.question
...
def test_gui_prefill_default() -> None:
    ...
    by_name = {field.short_question: field for field in bridge.last.fields}
...
def test_gui_prefill_choice() -> None:
    ...
    dialect = bridge.last.fields[_spec_index('csv.dialect')]
    assert isinstance(dialect, AskChoiceField)
...
def test_form_bridge_answers() -> None:
    """A bridge that submits a whole form configures the endpoint."""
    file_access = FileAccess.CREATE
    forms: list[dict[str, object]] = [
        {'csv.dialect': 'UNIX', 'csv.delimiter': ';'}]
    bridge = _FormReturnBridge(_format_index('CSV', file_access), forms)
...
def test_form_validator() -> None:
    """The wizard hands the form bridge a validator that blocks values.

    A bridge that shows all options at once, such as the Textual bridge
    or a graphical one, checks answers while the form is open through
    this validator, so the wizard has to supply a working one.
    """
    file_access = FileAccess.CREATE
    bridge = _FormReturnBridge(_format_index('CSV', file_access), [{}])
    tio_json_config_wizard(access_capabilities(file_access), file_access,
                           bridge)
    validator = bridge.last.validator
    assert validator is not None
    bad = validator(_csv_answers({'csv.delimiter': ';;'}), 0)
    assert bad.is_valid is False
    assert validator(_csv_answers({'csv.delimiter': ';'}), 0).is_valid is True
